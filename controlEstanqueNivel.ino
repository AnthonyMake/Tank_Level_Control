...
const int valvula  = 8;            //Actuator Pin (fan)
...
PID myPID(&Pv, &Mv, &Sp, kp, ki, kd, DIRECT);

void setup() {
  
  Serial.begin(9600);
  
  Pv = unisonicMeas();                          //initial PV Value to trigger the actuator.
  Sp = map(analogRead(potPin),0,1023,0,15);     //initial SetPoint
...
void loop() {
  ...
  Sp = map(analogRead(potPin),0,1023,0,15);
  Pv = usonicMeas();
  
  myPID.Compute();
  
  analogWrite(valvula,Mv);
